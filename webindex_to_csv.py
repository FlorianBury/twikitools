...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Convert a WebIndex html page to csv (tab-separated plain text, printed to stdout)")
    ...
    from twikitools.parse import webindex_entries
    with open(getattr(args, "webindex.html"), encoding=args.encoding) as fp:
        soup = BeautifulSoup(fp, "lxml")
        for web_topic, rev, author, summary in webindex_entries(soup):
